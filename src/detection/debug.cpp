              ...
              const int channel, const float in_value = 255.f)
{
    ...
    for (int row = start_row; row < end_row; ++row)
    {
        for (int col = start_col; col < end_col; ++col)
        {
            ...
            const auto [val, inside] = calib::board::accurate_intensity(row, col, row_center, col_center,
                                                                        radius_in_pixel, in_value, image(row, col)[0]);
            if (inside)
            {
...
{
void debug::save_inner_markers_and_unique(const cv::Mat1b &image, const std::vector<MarkerUnidentified> &markers_core,
                                          ...
                                          const size_t scale_idx)
{
    ...
    {
        if (marker.type_ == Type::INNER_CORE)
        {
    ...
    for (const auto &marker : ring_and_coding)
    {
        ...
        else if (marker.type_ == Type::CODING)
        {
    ...
    io::debug::save_image(painted, std::format("soup_{}_{}", image_idx, scale_idx));
}

void debug::save_inner_markers_and_unique(const cv::Mat1b &image, const std::vector<MarkerCoding> &coding,
                                          const std::vector<MarkerRing> &ring, const int image_idx,
                                          const size_t scale_idx)
{
    cv::Mat3b painted;
    cv::cvtColor(image, painted, cv::COLOR_GRAY2BGR);
    ...
    io::debug::save_image(painted, std::format("final_{}_{}", image_idx, scale_idx));
}
                                       ...
                                       const std::vector<MarkerRing> &markers, const int image_idx)
{
    ...
    io::debug::save_image(painted, std::format("identified_{}", image_idx));
}

void debug::save_neighbors_edges(const cv::Mat1b &image, const std::vector<MarkerNeighborhood> &neighbors,
                                 ...
                                 const int image_idx)
{
    ...
    io::debug::save_image(painted, std::format("neighbors_{}", image_idx));
}
}  // namespace calib::marker
