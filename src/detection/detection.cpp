...
namespace
{
constexpr bool kShowMarkers = true;

constexpr float kMaxSaturation = 1.f;  // 1 is OFF, (0,1) is ON

cv::Mat1b binarize(cv::Mat1b &input, const calib::marker::DetectionParameters &parameters)
{
...
std::vector<calib::MarkerUnidentified> filter_as_objects_simple_descriptors_cores(
    cv::Mat1b &input, cv::Mat1b &binarized, const calib::marker::DetectionParameters &parameters)
{
    cv::Mat1i labels, stats;
    cv::Mat1d centroid;
    cv::connectedComponentsWithStats(binarized, labels, stats, centroid);

    std::vector<calib::MarkerUnidentified> markers;
    ...
    for (int object_idx = 0; object_idx < stats.rows; ++object_idx)
    {
        ...
        const auto type = calib::marker::symetries::test_intensities_values_and_symmetries_cores(
        ...
        if (type == calib::Difference::INNER_BRIGHTER)
        {
            const auto [min, max] =
            ...
            markers.emplace_back(object_idx + 1, centroid(object_idx, 1), centroid(object_idx, 0),
                                 calib::Type::INNER_CORE, stats(object_idx, cv::CC_STAT_WIDTH),
...
std::vector<calib::MarkerUnidentified> filter_as_objects_simple_descriptors_rings(
    cv::Mat1b &input, cv::Mat1b &binarized, const calib::marker::DetectionParameters &parameters)
{
    cv::Mat1i labels, stats;
    cv::Mat1d centroid;
    ...
    for (int object_idx = 0; object_idx < stats.rows; ++object_idx)
    {
        ...
        const auto type = calib::marker::symetries::test_intensities_values_and_symmetries_rings(
        ...
        switch (type)
        {
            case calib::Difference::OUTER_BRIGHTER:
            {
                const auto [min, max] =
                    min_max_in_rect(input, centroid(object_idx, 1), centroid(object_idx, 0),
                                    stats(object_idx, cv::CC_STAT_WIDTH), stats(object_idx, cv::CC_STAT_HEIGHT));
                ...
                markers.emplace_back(object_idx + 1, centroid(object_idx, 1), centroid(object_idx, 0),
                                     calib::Type::CODING, stats(object_idx, cv::CC_STAT_WIDTH),
                                     stats(object_idx, cv::CC_STAT_HEIGHT), min, max);
                break;
            }
            case calib::Difference::NO_DIFFERENCE:
            {
                const auto [min, max] =
                    min_max_in_rect(input, centroid(object_idx, 1), centroid(object_idx, 0),
                                    stats(object_idx, cv::CC_STAT_WIDTH), stats(object_idx, cv::CC_STAT_HEIGHT));
                markers.emplace_back(object_idx + 1, centroid(object_idx, 1), centroid(object_idx, 0),
                                     calib::Type::RING, stats(object_idx, cv::CC_STAT_WIDTH),
                                     stats(object_idx, cv::CC_STAT_HEIGHT), min, max);
                break;
            }
            case calib::Difference::INNER_BRIGHTER:
            default:
                break;
        }
    }
...
std::pair<std::vector<calib::MarkerCoding>, std::vector<calib::MarkerRing>> symetric_prune(
    std::vector<calib::MarkerUnidentified> &inner, std::vector<calib::MarkerUnidentified> &rings_and_unique)
{
    ...
    std::vector<calib::MarkerRing> ring_with_centers;
...
void remove_unused_markers(Eigen::Matrix<std::optional<int>, -1, -1> &ordering, std::vector<calib::MarkerRing> &ring)
{
    ...
    std::vector<calib::MarkerRing> rings_shrink;
...
void assign_global_ids(const Eigen::Matrix<std::optional<int>, -1, -1> &ordering,
                       std::vector<calib::MarkerRing> &marker, const calib::BoardHexGrid &board)
{
    for (int row = 0; row < ordering.rows(); ++row)
    {
        for (int col = 0; col < ordering.cols(); ++col)
...
cv::Mat1b create_marker_area(const std::vector<calib::MarkerRing> &rings, const int rows, const int cols)
{
...
cv::Mat1b create_calibrated_area(const std::vector<calib::MarkerRing> &rings, const calib::BoardHexGrid &board,
...
namespace calib
{
std::optional<ImageDecoding> marker::detect_and_identify(cv::Mat1b &input, const DetectionParameters &parameters,
                                                         const BoardHexGrid &board, const int image_idx)
{
    const int num_pixels = input.rows * input.cols;
    const int num_valid = cv::countNonZero(input < 254);
    const float ratio_saturated = 1.f - float(num_valid) / float(num_pixels);
    if (ratio_saturated > kMaxSaturation)
    {
        // return std::nullopt;
    }

    std::vector<MarkerCoding> best_coding;
    std::vector<MarkerRing> best_rings;

    cv::Mat1b best_input = input.clone();
    cv::Mat1b best_binarized;
    cv::Mat1b best_inverted_binarization;
    std::vector<MarkerUnidentified> best_inner;
    std::vector<MarkerUnidentified> best_ring_and_coding;
         ...
         ++brightness_scale_idx)
    {
        ...
        if (brightness_scale != 1.0f)
        {
            ...
            const int num_valid_copy = cv::countNonZero(input_copy < 254);
            if (float(num_valid_copy) / float(num_pixels) < 0.8f)
            {
                continue;
            }
        }
    ...
    std::vector<MarkerCoding> coding = best_coding;
    std::vector<MarkerRing> rings = best_rings;
    ...
    if constexpr (kShowMarkers)
    {
        io::debug::save_image(marker_area, std::format("marker_area_{}", image_idx));
        io::debug::save_image(calibrated_area, std::format("calibrated_area_{}", image_idx));
    }

    return ImageDecoding(input, binarized, inverted_binarization, decoding->ordering(), decoding->markers, marker_area,
                         calibrated_area);
}
}  // namespace calib
