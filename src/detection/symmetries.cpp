...
namespace calib::marker::symetries
{
Difference test_intensities_values_and_symmetries_cores(const cv::Mat1b &image, const int row_center,
                                                        ...
                                                        const float min_difference_scale)
{
    ...
    {
        return Difference::INNER_BRIGHTER;
    }
    else if (!left_brighter && !right_brighter && !up_brighter && !down_brighter)
    {
        return Difference::OUTER_BRIGHTER;
    }
    return Difference::NO_DIFFERENCE;
}

Difference test_intensities_values_and_symmetries_rings(const cv::Mat1b &image, const int row_center,
                                                        const int col_center, const int width, const int height,
                                                        const float min_difference_scale)
{
