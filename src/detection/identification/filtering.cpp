...
std::set<Edge> get_obtuse_edges(const std::vector<calib::MarkerNeighborhood> &adj)
{
    static constexpr float kMaxObtuseAngle = 1.1f * std::numbers::pi / 2.f;
    std::set<Edge> out;
...
std::set<Edge> get_vertex_without_neighbors_knowing_each_other(const std::vector<calib::MarkerNeighborhood> &graph)
{
...
std::set<Edge> get_neighbors_dont_know_each_other(const std::vector<calib::MarkerNeighborhood> &adj)
{
...
std::set<Edge> get_single_neighbour_vertice_edges(const std::vector<calib::MarkerNeighborhood> &adj)
{
...
std::set<Edge> get_neighbors_that_dont_know_me(const std::vector<calib::MarkerNeighborhood> &adj)
{
...
void sort_neighbors_by_angle(std::vector<calib::MarkerNeighborhood> &graph)
{
...
void check_size(const std::vector<calib::MarkerNeighborhood> &graph)
{
    for (size_t id_1st = 0; id_1st < graph.size(); ++id_1st)
    {
...
std::pair<std::set<Edge>, std::set<std::array<Edge, 2>>> get_set_of_longest_edge_of_4_neighbors(
    const std::vector<calib::MarkerNeighborhood> &graph)
{
...
std::set<Edge> get_incorrect_crossing_edges(const std::vector<calib::MarkerNeighborhood> &graph,
                                            const std::set<std::array<Edge, 2>> &crossing_edge_pairs)
{
    static constexpr float kAngleThresh = 5.f * (std::numbers::pi / 180.f);
        ...
        for (int idx = 0; idx < 4; ++idx)
        {
            ...
            for (int idx_5th = 0; idx_5th < (int)graph[id].neighbors.size(); ++idx_5th)
            {
                ...
                for (int idx_6th = 0; idx_6th < (int)graph[id_5th].neighbors.size(); ++idx_6th)
                {
                    ...
                    const float angle_6th_to_1st_pi = unroll_angle(angle_6th - angle_a + std::numbers::pi);
                    const float angle_6th_to_2nd = unroll_angle(angle_6th - angle_b);
                    const float angle_6th_to_2nd_pi = unroll_angle(angle_6th - angle_b + std::numbers::pi);
...
// having an angle ordering find edges which rotated 180 degrees does not have corresponding edge
std::set<Edge> get_edges_without_collinear_edges(const std::vector<calib::MarkerNeighborhood> &graph)
{
    static constexpr float kAngleThresh = 5.f * (std::numbers::pi / 180.f);
    static constexpr float kDistDiffRatioThresh = 0.07f;
        ...
        for (int idx_2nd = 0; idx_2nd < (int)graph[id_1st].neighbors.size(); ++idx_2nd)
        {
            ...
            for (int idx_3rd = 0; idx_3rd < (int)graph[id_1st].neighbors.size(); ++idx_3rd)
            {
                ...
                for (int idx_4th = 0; idx_4th < (int)graph[id_1st].neighbors.size(); ++idx_4th)
                {
                    ...
                    const float angle_4th_to_2nd = unroll_angle(angle_2nd - angle_4th + std::numbers::pi);
                    const float angle_4th_to_3rd = unroll_angle(angle_3rd - angle_4th + std::numbers::pi);
                    ...
                    for (int idx = 0; idx < 2; ++idx)
                    {
                        ...
                        for (int idx_5th_in = 0; idx_5th_in < (int)graph[id_1st_or_4th].neighbors.size(); ++idx_5th_in)
                        {
                            ...
                            const float angle_3th_to_5th_pi =
                                unroll_angle(angle_5th_in_4th - angle_4th + std::numbers::pi);
...
std::set<Edge> get_opposite_edges_with_different_length(const std::vector<calib::MarkerNeighborhood> &graph)
{
    static constexpr float kAngleThresh = 5.f * (std::numbers::pi / 180.f);
    ...
    for (int id_1st = 0; id_1st < (int)graph.size(); ++id_1st)
    {
        ...
        for (auto it_edge_a = surrounding_edges_1st.begin(); it_edge_a != surrounding_edges_1st.end(); ++it_edge_a)
        {
            ...
            for (auto it_edge_b = std::next(it_edge_a, 1); it_edge_b != surrounding_edges_1st.end(); ++it_edge_b)
            {
                ...
                const float angle_diff_pi = std::abs(unroll_angle(angle_b + std::numbers::pi - angle_a));
...
void remove_edge(std::vector<calib::MarkerNeighborhood> &graph, const Edge &edge)
{
...
void remove_edges(std::vector<calib::MarkerNeighborhood> &graph, const std::set<Edge> &edges_to_remove)
{
...
void identification::filter_out_flase_positives(std::vector<MarkerNeighborhood> &neighbors_graph)
{
    while (true)
    {
