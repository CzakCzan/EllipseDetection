...
static constexpr float kAngleThresh = 5. * (std::numbers::pi / 180.f);
...
static constexpr std::array<std::array<std::pair<int, float>, 2>, 3> kHexAxisDirectionOffsetsLeft{
    // {Q+/R+/S+}, {Q-/R-/S-}
    {{std::pair(1, -std::numbers::pi), std::pair(-2, 0.f)},
     {std::pair(0, 0.f), std::pair(-3, -std::numbers::pi)},
     {std::pair(-1, std::numbers::pi), std::pair(2, 0.f)}}};
...
static constexpr std::array<std::array<std::pair<int, float>, 2>, 3> kHexAxisDirectionOffsetsRight{
    // {Q+/R+/S+}, {Q-/R-/S-}
    {{std::pair(1, 0.f), std::pair(-2, -std::numbers::pi)},
     {std::pair(0, std::numbers::pi), std::pair(-3, 0.f)},
     {std::pair(-1, 0.f), std::pair(2, std::numbers::pi)}}};
...
std::optional<float> get_axis_rotation_when_all_6_detected(const std::vector<calib::MarkerNeighborhood> &graph,
                                                        ...
                                                        const std::vector<float> &angles)
{
    ...
    for (size_t idx_1st = 0; idx_1st < neighbors.size(); ++idx_1st)
    {
        for (size_t idx_2nd = 0; idx_2nd < neighbors.size(); ++idx_2nd)
        {
            ...
            const float abs_diff_pi = std::abs(abs_diff - std::numbers::pi);
...
std::array<std::optional<float>, 3> get_axis_rotations_when_at_least_one_missing(
    const std::vector<calib::MarkerNeighborhood> &graph, const int id_coding, const int id_middle_ring)
{
    auto coding_neighbors = graph[id_coding].neighbors;
    auto coding_angles = graph[id_coding].angles;
    const auto &middle_neighbors = graph[id_middle_ring].neighbors;
    ...
    {
        const float angle_middle =
            unroll_angle(coding_angles[idx_middle_ring_in_coding] + (kIsCodingLeft ? 0.f : std::numbers::pi));
        ...
        if (std::abs(diff) > kAngleThresh && std::abs(diff) - kAngleThresh < std::numbers::pi / 2.f)
        {
            angle = coding_angles[idx] + std::numbers::pi;
            diff = angle - angle_middle;
        }
        if (std::abs(diff) + kAngleThresh > std::numbers::pi)
        {
            angle = coding_angles[idx] - std::numbers::pi;
            diff = angle - angle_middle;
        }
    ...
    {
        if (angles_to_middle[idx] < -std::numbers::pi / 2.f)
        {
            found_axes[HexAxis::Q].push_back(idx);
        }
        else if (angles_to_middle[idx] > std::numbers::pi / 2.f)
        {
...
template <bool kIsCodingLeft>
std::array<std::optional<float>, 3> get_axis_rotations(const std::vector<calib::MarkerNeighborhood> &graph,
                                                       const int id_coding, const int id_middle_ring)
{
                                       ...
                                       const float angle_thresh)
{
    ...
    for (const float angle_2nd : angles)
    {
        ...
        else if (angle_thresh > std::abs(angle_2nd + std::numbers::pi - angle_1st.value()))
        {
            possible_angles.push_back(angle_2nd + std::numbers::pi);
        }
        else if (angle_thresh > std::abs(angle_2nd - std::numbers::pi - angle_1st.value()))
        {
            possible_angles.push_back(angle_2nd - std::numbers::pi);
...
void print_axial_stats(const std::vector<calib::MarkerNeighborhood> &graph, const std::vector<AxialCoord> &axial_coords)
{
...
template <bool kIsCodingLeft>
std::vector<AxialCoord> make_axial_coords(const std::vector<calib::MarkerNeighborhood> &graph, const int id_coding,
                                          const int id_middle_ring)
{
    ...
    while (!bfs_queue.empty())
    {
        ...
        for (size_t idx_b = 0; idx_b < graph[id_a].neighbors.size(); ++idx_b)
        {
            ...
            else if (axis_q_a && (kAngleThresh > std::abs(angle_b - std::numbers::pi - axis_q_a.value()) ||
                                  kAngleThresh > std::abs(angle_b + std::numbers::pi - axis_q_a.value())))
            {  // Q-
                axis_q_b = (kAngleThresh > std::abs(angle_b - std::numbers::pi - axis_q_a.value()))
                               ? angle_b - std::numbers::pi
                               : angle_b + std::numbers::pi;
                q_b = q_a;
                r_b = r_a + 1;
                s_b = s_a - 1;
            ...
            else if (axis_r_a && (kAngleThresh > std::abs(angle_b + std::numbers::pi - axis_r_a.value()) ||
                                  kAngleThresh > std::abs(angle_b - std::numbers::pi - axis_r_a.value())))
            {  // R-
                axis_r_b = (kAngleThresh > std::abs(angle_b + std::numbers::pi - axis_r_a.value()))
                               ? angle_b + std::numbers::pi
                               : angle_b - std::numbers::pi;
                q_b = q_a - 1;
                r_b = r_a;
            ...
            else if (axis_s_a && (kAngleThresh > std::abs(angle_b + std::numbers::pi - axis_s_a.value()) ||
                                  kAngleThresh > std::abs(angle_b - std::numbers::pi - axis_s_a.value())))
            {  // S-
                axis_s_b = (kAngleThresh > std::abs(angle_b + std::numbers::pi - axis_s_a.value()))
                               ? angle_b + std::numbers::pi
                               : angle_b - std::numbers::pi;
                q_b = q_a + 1;
                r_b = r_a - 1;
...
std::vector<int> assign_global_indices(const std::vector<std::optional<OffsetCoord>> &offset_coords,
                                       const calib::BoardHexGrid &board)
{
...
std::optional<int> get_common_neighbour(const std::vector<calib::MarkerNeighborhood> &graph,
...
std::vector<int> identification::create_markers_order(const std::vector<calib::MarkerNeighborhood> &graph,
