...
std::vector<calib::MarkerNeighborhood> find_neighbors(const std::vector<calib::MarkerRing> &markers)
{
    static constexpr float kAngleThresh = 0.5f * std::numbers::pi / 6.f;
    ...
    std::vector<calib::MarkerNeighborhood> neighbors_graph(markers.size());
...
std::pair<int, int> swap_coding_pair(const std::pair<int, int> coding_pair,
                                     const std::vector<calib::MarkerRing> coding_then_rings)
{
...
std::vector<std::pair<int, int>> get_coding_pairs(const std::vector<calib::MarkerNeighborhood> &neighbors_graph,
...
std::optional<std::pair<int, int>> define_initial_ordering_set(
    const std::vector<calib::MarkerNeighborhood> &neighbors_graph, const size_t num_found_coding,
...
auto concat_coding_rings(const std::vector<calib::MarkerRing> &rings, const std::vector<calib::MarkerCoding> &coding)
{
    std::vector<calib::MarkerRing> coding_then_rings;

    coding_then_rings.reserve(rings.size() + coding.size());
    for (size_t marker_idx = 0; marker_idx < coding.size(); ++marker_idx)
    {
        const calib::MarkerCoding &src = coding[marker_idx];
        calib::MarkerRing dst;
        dst.row_ = src.row_;
        dst.col_ = src.col_;
...
calib::identification::OrderingBoardHex create_ordering(const std::vector<int> &markers_order,
                                                        const calib::BoardHexGrid &board)
{
    calib::identification::OrderingBoardHex ordering_set;
...
Eigen::Matrix<std::optional<int>, -1, -1> &identification::Decoded::ordering() { return ordering_hex.ordering_; }

std::pair<std::optional<identification::Decoded>, std::vector<MarkerNeighborhood>> identification::assign_global_IDs(
    const std::vector<MarkerCoding> &coding, const std::vector<MarkerRing> &rings, const BoardHexGrid &board)
{
    std::pair<std::optional<Decoded>, std::vector<MarkerNeighborhood>> ret;
    auto &[decoded, neighbors] = ret;

    std::vector<calib::MarkerRing> coding_then_rings = concat_coding_rings(rings, coding);
