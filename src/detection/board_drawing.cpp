...
std::pair<float, float> on_image_location(const calib::BoardHexGrid &board, const int row, const int col,
                                          const float cm_per_pixel)
{
                 ...
                 const float radius_in_pixel, const float in_value, const float out_value)
{
    ...
    for (int row = start_row; row < end_row; ++row)
    {
        for (int col = start_col; col < end_col; ++col)
        {
            ...
            const auto [val, inside] = calib::board::accurate_intensity(row, col, circle_row_center, circle_col_center,
...
void draw_footer(cv::Mat1b &image, const calib::BoardHexGrid &board, const float cm_per_pixel,
                 const float footer_height_cm = .2f, const int footer_thickness = 10)
{
    ...
    const std::string footer = std::format(
        "{}x{} | Coding: ({},{}), ({},{}) | Spacing: {:0.3f} | Outer: {:0.3f} | Inner: "
        "{:0.3f} | Is even: {}",
    ...
    cv::putText(image, footer,
                cv::Point((board.top_left_(0) + board.outer_radius_) / cm_per_pixel,
                          (board.top_left_(1) - footer_height_cm / 2.f - .7f) / cm_per_pixel),
                                      ...
                                      const cv::Size_<float> board_dimension_mm)
{
    ...
    const int rows = board_dimension_mm.height / cm_per_pixel;
    const int cols = board_dimension_mm.width / cm_per_pixel;
